...
模型选择器只在 Chat Completions mode 可用，Responses mode 不显示。服务首次需要时通过 compatible provider 的 `GET /models` 读取所有返回且合法的模型 ID，并始终加入启动参数指定的默认模型；不按名称或推测的能力过滤。用户可以手动刷新，刷新失败时保留上一次成功列表，首次获取失败时仍允许使用默认模型。选择结果只影响下一次提交：发送后当前 run 的模型不可变，同一会话的 follow-up 可以重新选择。页面刷新后恢复启动默认模型，不在浏览器中持久化；模型目录请求和实际模型调用都由本地服务完成，浏览器不会接触 API key 或 base URL。compatible provider 除 Chat Completions 工具调用约定外，还需提供与 OpenAI SDK 分页迭代兼容的 `GET /models` 响应；不满足时会降级到默认模型，而不会自动探测或切换 API mode。

页面顶部用文件夹图标显示当前工作区绝对路径；路径过长时单行省略，悬停可查看完整值。该绝对路径只投影到当前 loopback 页面，并带 `no-store` 响应约束；它不写入 REST、SSE、SQLite、JSONL 或模型上下文。助手已经完成的回复若包含闭合的 fenced code，代码块右上角会显示“复制”按钮，只复制围栏内的原始代码；成功时短暂显示“已复制”，失败时只显示固定提示。运行期间只有当前活动回复左侧显示轻量三点动画，它只表示 run 仍在进行，不代表真实进度；系统启用“减少动态效果”后循环动画会停止，历史消息和终态卡片始终保持静态。
...
可信验证结果都会进入状态和审计，但只有首次证据、validation index 前进、状态变化或来源从本地完整性升级为模型/用户验证时才推进进度；重复同级证据不能反复清除 checkpoint。修改批次通过本地完整性后立即进入 `post_mutation_integrity` checkpoint，再使用上述 Standard 1 / Deep 2 只读额度收敛。
...
两个模型适配器内部都支持 provider-neutral 文本流事件。Chat Completions 流在尚未向用户公开文本时若结构非法，可在同一 logical call 内最多执行一次同步请求；已经公开文本后不会同步回退。**CLI 仍使用同步最终报告**，不会逐 token 显示内容；Web GUI 通过经过认证的 fetch-SSE 投影安全事件。部分输出仅驻留内存；中断后不会写入消息历史、JSONL 或 FinalReport。

项目自行创建的 Responses 与 Chat Completions SDK client 都设置 30 秒单次网络操作 timeout，关闭 SDK 自带重试；适配器的确定性重试仍计入现有 provider attempt 预算。模型响应同时包含文本与工具调用时，文本只作为临时叙述和内部调用配对存在：首个工具启动前 GUI 会清除它，SQLite 不会保存；只有不含工具调用的最终文本才成为会话回复。
...
| `create_directory` | 一次只创建一级目录，不递归创建父目录；直接父目录必须已经存在。 |
...
GUI 的命令活动卡会在存在整数进程退出码时显示 `exit N`；例如命令启动成功但测试退出 1 时显示 `exit 1`，而不是工具调度状态 `ok`。没有进程退出码的文件工具仍显示 `ok`、`error` 或 `rejected`。
...
用户提供 `--verify` 时，即使已有新鲜 Java 证据，也必须执行用户命令并以其结果为最终门槛；未提供时才允许可信 `run_command`、完整通过的 Java verification 证据或内建完整性验证。已有模型/用户验证因后续修改而过期时不得降级为完整性检查。没有这类过期证据时，符合条件的修改批次结束后会执行一次本地结构完整性检查；它只证明 changed path 仍安全存在且内容结构可读取，不代表测试、编译或用户指定验证已经通过。内建检查接受安全目录；对文件则限制为 524,288 原始字节和 UTF-8，`.py`、`.json`、`.toml` 还必须通过语法解析，其他文本（包括 C/C++）不表示已编译或运行。
...
MiniCodex 不提供 PTY、键盘/终端交互自动化、C/C++ 编译器或 Bash/WSL 逃逸路径。交互程序的问题应在现有工具能覆盖时写成正式适配器或输入层回归测试；语法检查和纯领域单元测试不能证明真实交互可用。无法自动覆盖时，最终报告必须明确说明人工交互仍未验证。不得仅为绕过命令策略创建一次性诊断脚本。
