...
- 包布局：生产代码放在 `src/coding_agent/`，按 `application`、`engine`、
  `providers`、`operations`、`sessions`、`skills`、`web` 七个职责分包；
  测试放在 `tests/` 并按相同职责组织。
...
生产模块位于 `src/coding_agent/`，按职责划分：

| 子包 | 职责 | 主要依赖方向 |
| --- | --- | --- |
| `application/` | 一次性 CLI 的参数解析、配置装配和顶层应用入口 | 调用核心、Provider、会话和工具层 |
| `engine/` | Agent 主循环、消息与状态、上下文、预算、进度、验证、终止、日志和报告 | 只依赖抽象协议及受控工具接口 |
| `providers/` | Responses、compatible Chat Completions 适配与模型目录查询 | 实现 `engine` 定义的模型协议 |
| `operations/` | 路径和命令安全策略；文件、Shell、Java 等本地工具及其注册执行 | 供 `engine` 与应用层调用 |
| `sessions/` | 会话持久化、运行控制、事件投影、删除和恢复 | 组合核心运行能力，不改变安全边界 |
| `skills/` | 声明式 Skill 目录读取、校验和 ZIP 包导入 | 只提供指令数据，不增加工具权限 |
| `web/` | 本地 FastAPI 服务、认证、Web 启动入口和静态界面资源 | 调用应用、会话和 Skill 能力 |

`tests/` 使用相同的职责目录，另保留 `integration/`、`js/` 和 `manual/`
分别承载端到端 Python 回归、浏览器逻辑测试和显式人工验证夹具。
这次目录迁移只改变内部导入路径与打包资源位置；`coding-agent`、
`coding-agent-web` 命令名、公共 HTTP 行为、安全策略、验证门槛和持久化
格式均保持不变。旧任务记录中的历史路径仅描述当时的实现位置，不作为
当前导入路径。
