...
from coding_agent.application.cli import build_parser, main
from coding_agent.application.config import ApiMode, ConfigError, RunConfig, load_run_config
...
def test_standard_console_command_rejects_missing_key_before_app(
    ...
    assert "coding_agent.application.app" not in completed.stderr
...
def test_config_routes_workspace_validation_through_path_guard(
    ...
    monkeypatch.setattr("coding_agent.application.config.PathGuard", RejectingPathGuard)
...
def guarded_import(name, globals=None, locals=None, fromlist=(), level=0):
    if name in {{'coding_agent.engine.agent', 'coding_agent.engine.model'}}:
        raise AssertionError('agent/model imported before verify authorization')
    return real_import(name, globals, locals, fromlist, level)
builtins.__import__ = guarded_import
from coding_agent.application.cli import main
code = main(
